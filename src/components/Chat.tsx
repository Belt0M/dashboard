...
const Chat = () => {
	...
	return (
		<div
			className='flex flex-col justify-between fixed md:right-52 md:top-16 md:w-72 md:h-3/5 h-3/4 w-3/4 top-1/2 right-1/2 translate-x-1/2 -translate-y-1/2 md:translate-x-0 md:translate-y-0 bg-white z-10 rounded-tl-md rounded-bl-md rounded-br-md p-2'
			style={{
		...
		>
			<WidgetsHeader headerText='Messages' widgetType={'chat'} />
			<div className='h-2/3 p-3 mb-9'>
				{chatData.map((el, index) => (
					<div
						className='flex items-center gap-4 pb-2 mt-2 border-b-gray-500/50 border-b-1'
						key={index}
					>
						<img
							src={el.image}
						...
						<div>
							...
							<p className='text-gray-400 text-xs'>{el.time}</p>
						</div>
					</div>
				))}
			</div>
